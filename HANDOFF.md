...
| Commit | Phase |
|---|---|
| (this branch) | **§A2 follow-up: Field.offset / Field.size / Field.alignment** — the reflection Field struct grew three Int members; the comptime folder populates each entry of `T.fields[i]` with the per-field offset (C-style padding) plus its own size + alignment. Sema eager-folds any member access on a Field value so `T.fields[i].offset` lands in C++ source as a literal. Str / StrConst gained primitive layout (16-byte std::string_view, 8-byte align). |
